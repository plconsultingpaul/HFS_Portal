...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { buttonId, executeParameters, userId, flowNodeId, continueFromNode, userConfirmationResponse, userInputResponse, pendingContextData, currentGroupNodeId, existingContextData } = requestData;
    ...
    if (button.has_flow) {
      ...
      if (userInputResponse && pendingContextData) {
        console.log('🔄 Continuing from user input...');
        ...
        contextData.execute.input[userInputResponse.variableName] = userInputResponse.value;
        console.log(`📝 Stored input value at execute.input.${userInputResponse.variableName}:`, userInputResponse.value);
            ...
            switch (node.step_type) {
              ...
              case 'route': {
                ...
                if (matchedRouteIndex >= 0) {
                  const handleId = `route_${matchedRouteIndex}`;
                  const matchedEdge = flowEdges.find(e => e.source_node_id === node.id && e.source_handle === handleId);
                  if (matchedEdge) {
                    contextData.lastEdgeHandle = handleId;
                    const targetIdx = executionOrder.findIndex(n => n.id === matchedEdge.target_node_id);
                    if (targetIdx !== -1 && targetIdx > i) {
                      console.log(`\u27A1\uFE0F Following route "${routeOptions[matchedRouteIndex]?.label}" to node at index ${targetIdx}`);
                      i = targetIdx - 1;
                    }
                  } else {
                    console.log(`\u26A0\uFE0F No edge found for handle ${handleId} - ending flow`);
                    i = executionOrder.length;
                  }
                } else {
                  console.log('\u26A0\uFE0F No route matched and no default set - ending flow');
                  i = executionOrder.length;
                }
                break;
              }
              ...
              case 'user_input': {
                const config = step.config_json || {};
                const inputLabel = replaceVariables(config.inputLabel || 'Provide Input', contextData);
                console.log('📥 User input required:', config.inputType, inputLabel);
