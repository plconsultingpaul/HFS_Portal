...
import { evaluateFunction, evaluateAddressLookupAsync, type FunctionLogic } from './lib/functionEvaluator.ts';
...
async function processEmail(
  ...
  try {
    ...
    if (matchingRule.processing_mode === 'workflow_v2' && matchingRule.workflow_v2_id) {
      ...
      const v2Resp = await fetch(`${supabaseUrl}/functions/v1/json-workflow-processor-v2`, {
        method: 'POST',
        headers: {
      ...
      return buildResult(emailId, fromEmail, subject, receivedDate, processedSuccessfully, errorMessage, extractionLogId, [], matchingRule, null);
    }
