/*
  # Add User Input Step Type to Flow Nodes

  1. Changes
    - Adds 'user_input' to the allowed step_type values on `execute_button_flow_nodes`
    - User Input provides interactive input collection (barcode scanner, number pad)
    - Pauses flow execution, collects value from user, stores in context data
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'execute_button_flow_nodes') THEN
    ...
    CHECK ((step_type IS NULL) OR (step_type = ANY (ARRAY[
      ...
      'multipart_form_upload', 'route', 'user_input'
    ])));
  END IF;
END $$;