...
import { FileText, LogOut, User, HelpCircle, Menu, DollarSign, Users as UsersIcon, BookUser, MapPin, Receipt, Building2, KeyRound } from 'lucide-react';
        ...
        <div className="p-3 border-t border-orange-100 dark:border-gray-700 flex-shrink-0 bg-white/90 dark:bg-gray-800/90">
          {isSidebarExpanded ? (
            <div className="transition-opacity duration-200 opacity-100">
              <div className="flex items-center gap-2 mb-3 px-3 py-2 bg-orange-50 dark:bg-orange-900/30 rounded-lg">
                <User className="h-4 w-4 text-orange-600 dark:text-orange-400 flex-shrink-0" />
                <span className="text-sm font-medium text-orange-700 dark:text-orange-300 truncate flex-1 min-w-0">
                  {user.username}
                </span>
                {user.isClientAdmin && (
                  <span className="h-5 w-5 rounded-full bg-orange-100 dark:bg-orange-800 text-orange-800 dark:text-orange-200 text-[10px] font-bold flex items-center justify-center flex-shrink-0" title="Admin">
                    A
                  </span>
                )}
                {onChangePassword && (
                  <button
                    ...
                    className="p-1 rounded-md text-orange-400 hover:text-orange-600 hover:bg-orange-100 dark:hover:text-orange-300 dark:hover:bg-orange-800/50 transition-colors flex-shrink-0"
            ...
            <div className="flex justify-center mb-3">
              <div className="p-3 bg-orange-50 dark:bg-orange-900/30 rounded-lg relative group cursor-pointer" title={`${user.username}${user.isClientAdmin ? ' (Admin)' : ''}`}>
                <User className="h-5 w-5 text-orange-600 dark:text-orange-400" />
                {onChangePassword && (
                  <button
                    onClick={() => setShowChangePassword(true)}
                    className="absolute -top-1 -right-1 p-1 rounded-full bg-white dark:bg-gray-700 shadow-sm text-orange-400 hover:text-orange-600 dark:hover:text-orange-300 transition-colors opacity-0 group-hover:opacity-100"
                    title="Change Password"
                  >
