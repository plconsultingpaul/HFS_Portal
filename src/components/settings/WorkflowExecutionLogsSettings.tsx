...
export default function WorkflowExecutionLogsSettings({
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await supabase.from('workflows_v2').select('id, name');
        if (data) {
  ...
  const getWorkflowName = (workflowId: string) => {
    ...
    if (v2WorkflowNames[workflowId]) return v2WorkflowNames[workflowId];
    return `Unknown Workflow (${workflowId.substring(0, 8)}...)`;
  };
