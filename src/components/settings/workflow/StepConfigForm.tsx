...
import { X, Plus, Trash2, Braces, HelpCircle, LogOut, Sparkles, FileText, AlertCircle, TextCursorInput } from 'lucide-react';
...
export default function StepConfigForm({ step, allSteps, apiConfig, onSave, onCancel, extractionType, executeButtonFields, arrayGroups = [] }: StepConfigFormProps) {
  ...
  const [userInputType, setUserInputType] = useState<'barcode_scanner' | 'number_pad'>('barcode_scanner');
    ...
    switch (stepType) {
      ...
      case 'user_input':
        config = {
          ...
          inputLabel: userInputLabel || (userInputType === 'barcode_scanner' ? 'Scan Barcode' : 'Enter Number'),
          variableName: userInputVariableName || 'userInputValue'
        };
        break;
    }
              ...
              <div className="space-y-4">
                  ...
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-sky-700 dark:text-sky-300 mt-1">
                        Pauses the flow and prompts the user to provide input via a barcode scanner or number pad.
                ...
                <div>
                  <Select
                    label="Input Type"
                    value={userInputType}
                    onValueChange={(value) => setUserInputType(value as 'barcode_scanner' | 'number_pad')}
                    options={[
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={userInputType === 'barcode_scanner' ? 'Scan Barcode' : 'Enter Phone Number'}
                  />
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                ...
                <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                  ...
                  <ul className="text-xs text-gray-600 dark:text-gray-400 space-y-1">
                    ...
                    <li>- The value is stored at <span className="text-sky-600 dark:text-sky-400 font-medium">execute.input.{userInputVariableName || 'scannedValue'}</span></li>
                  </ul>
                </div>
              </div>
            )}
